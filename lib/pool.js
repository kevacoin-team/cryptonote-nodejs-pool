...
// Kevacoin requires a reserve_size of 8.
let isKevacoin = config.coin == 'kevacoin';
let POOL_NONCE_SIZE = isKevacoin ? 8 : 16 + 1; // +1 for old XMR/new TRTL bugs
let EXTRA_NONCE_TEMPLATE = "02" + POOL_NONCE_SIZE.toString(16) + "00".repeat(POOL_NONCE_SIZE);
if (isKevacoin) {
	EXTRA_NONCE_TEMPLATE = "0208" + "00".repeat(POOL_NONCE_SIZE);
}
	...
	if (hashDiff.ge(blockTemplate.difficulty)) {
		...
		apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function (error, result) {
			if (error) {
				...
				let blockFastHash;
				if (isKevacoin) {
					let hashValue = cryptoNightFast(utils.cnUtil.convert_blob(shareBuffer));
					blockFastHash = reverseBuffer(hashValue).toString('hex');
				} else {
					blockFastHash = utils.cnUtil.get_block_id(shareBuffer, cnBlobType)
					.toString('hex');
				}
