...
function jsonHttpRequest (host, port, data, callback, path) {
	...
	// user and password are for Kevacoin only.
	var user = config.daemon.user;
	var password = config.daemon.password;
	...
	var options = {
		...
		auth: user + ':' + password, // Kevacoin requires username and password.
		headers: {
