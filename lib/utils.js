...
// For Kevacoin address valiadtion.
...
// Kevacoin doesn't need addressBase58Prefix.
let addressBase58Prefix = (!isKevacoin) && parseInt(cnUtil.address_decode(Buffer.from(config.poolServer.poolAddress))
	.toString());
...
exports.validateMinerAddress = function (address) {
	if (isKevacoin) {
		// Kevacoin validates address differently.
		return validateKevaAddress(address);
	}
